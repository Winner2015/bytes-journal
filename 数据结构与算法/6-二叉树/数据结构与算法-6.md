...
通过前面的学习，我们知道，有序数组可以利用二分查找法快速的查找特定的值，时间复杂度为`O(log2N)`，但是插入数据时很慢，时间复杂度为`O(N)`；链表的插入和删除速度都很快，时间复杂度为`O(1)`，但是查找特定值很慢，时间复杂度为`O(N)`。
...
**如果树的每个节点最多有两个子节点，则称为二叉树**。如果节点的子节点可以多余两个，称为**多路树**。
