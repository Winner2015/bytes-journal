...
另外一个需要注意的前提是，纸牌需要随机、均匀地分配。MapReduce 非常害怕**数据倾斜**（Data Skew）——如果所有黑桃都分到了一个人手上，那他数牌的过程可能比其他人要慢很多，导致整个任务被拖慢。

如果有足够的人的话，问一些更有趣的问题就相当简单了。 比如”一摞牌的点数平均值是多少”。你可以通过合并“所有牌的值的和是什么”及“我们有多少张牌”这两个问题来得到答案。用这个和除以牌的张数就得到了平均值。

MapReduce算法的机制要远比这复杂得多，但是主体思想是一致的 ——**通过分散计算来分析大量数据**。MapReduce奠定了大数据分布式计算的基石，其核心思想至今仍深深影响着 Spark、Flink 等现代主流大数据处理框架。无论是早期的科技巨头如 Facebook、Google，还是如今的大数据生态，MapReduce 的思想始终是分析海量数据的底层逻辑。
...
例如，假设有一个巨大的二维数据集需要处理（比如求每个元素的开立方），其中对每个元素的处理是相同的，并且数据元素间不存在数据依赖关系，可以考虑不同的划分方法将其划分为子数组，由一组处理器并行处理。这其实只体现了 Map 的过程，如果需要将结果汇总，则还需要 Reduce 的过程。
...
WordCount，即统计一大批文件中每个单词出现的次数，经常被拿来当作MapReduce入门案例。主要处理流程如下：
...
- 减少 Map 端到 Reduce 端的网络传输数据量（网络IO）
...
MapReduce向用户提供了简单的编程接口，由框架层自动完成数据分布存储、数据通信、容错处理等复杂的底层处理细节，用户只需要使用接口实现自己的数据处理逻辑即可。也就是说你写一个分布式程序，跟写一个简单的串行程序是一模一样的，就是因为这个特点使得 MapReduce 编程变得非常流行。
